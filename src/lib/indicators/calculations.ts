...
export function calculateBollingerBands(
  ...
  for (let i = period - 1; i < bars.length; i++) {
    ...
    for (let j = 0; j < period; j++) {
      const diff = bars[i - j].close - sma;
      squaredDiffSum += diff * diff;
    }
    const stdDev = Math.sqrt(squaredDiffSum / period);

    const time = toChartTime(bars[i].timestamp);
...
export function calculateRSI(bars: Bar[], period: number = 14): IndicatorDataPoint[] {
  ...
  result.push({
    time: toChartTime(bars[period].timestamp),
    value: rsi,
  });
  ...
  for (let i = period; i < gains.length; i++) {
    ...
    result.push({
      time: toChartTime(bars[i + 1].timestamp),
      value: rsi,
    });
  }
...
export function calculateMACD(
  ...
  for (let i = 0; i < signalLine.length; i++) {
    histogram.push({
      ...
      value: macdLine[i + macdStart].value - signalLine[i].value,
    });
  }
...
export function calculateVolume(
  ...
  return bars.map((bar, i) => {
    const isUp = i === 0 || bar.close >= bars[i - 1].close;
    return {
